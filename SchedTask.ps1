...
Register-ScheduledTask -Action $action -TaskName "Daily Log Cleanup" -Description "This script will clear down Event Logs and DSC log files on a host and will be launched as a daily Scheduled Task" -RunLevel Highest -Settings $settings -TaskPath Housekeeping -Trigger $trigger -User System
